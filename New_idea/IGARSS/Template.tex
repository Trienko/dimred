...
The approach suggested by XX is, however, not a hypertemporal algorithm which implies that it does not fully exploit the temporal information contained witin the NDVI time-series it employs.\\

\noindent
In this paper, we propose a novel sequential (on-line) algorithm which uses a GMM to cluster MODIS time-series. As our proposed approach is sequential, it better utilises the information 
contained within the temporal dimension of the data which is being clustered than a non-sequential approach would. A supervised sequential classification algorithm which could discriminate 
...
The approach we present here uses the SPRT algorithm and a time-varying model which we construct by employing GMMs (i.e. in an unsupervised manner).\\
