...
We plot the reduced feature space obtained using the framework described in Section~\ref{sec:met} on the dataset described in Section~\ref{} in Figs.~\ref{fig:density_PCA} and \ref{fig:density_FFT}. The graphs in Figs.~\ref{fig:density_PCA} and \ref{fig:density_FFT} suggest that PCA outperforms the FFT method, i.e. the two 
classes are more separable in the reduced feature space when PCA is employed as a dimensionality reduction technique if compared to when the FFT techniques is used (wheter we incorporate the labels of the aformentioned dataset into our analysis or not). This observation is confirmed 
...
\begin{description}
 ...
 If the data is not seperable enough, or cannot be accurately represented by the chosen mixture model then the results that are produced by our framework when applied to 
 a unlabelled dataset is meaningless (this is the case for band 5, 6, 7 and NDVI). In contrast, if 
 the aforementioned conditions are met then the framework performs well and produces results similar to the case when the framework is applied to labelled data (this is the case for band
 1, 2, 3 and 4). The only major difference being: the FFT technique performs a little bit better than PCA in band 1.
\end{description}

\begin{itemize}
  \item  When we incorporate the labels into our analysis then the two reduction methods, PCA and FFT, perform similarly for these two bands.
 If we do not incorporate the labels, the FFT approach gives better results. This is because a GMM is not a good 
