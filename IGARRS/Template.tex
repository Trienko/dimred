...
% Title.
% ------
\title{AUTHOR GUIDELINES FOR IGARSS 2018 MANUSCRIPTS}
%
% Single address.
% ---------------
...
Assume that $\mathbf{X}_b$ is an $n\times t$ centered matrix (i.e. column means have been subtracted and are now equal to zero) which contains MODIS band $b$ data (to reduce clutter we will omitt the $b$ subscript in the rest of the paper), where $n$ denotes a MODIS pixel index and $t$ a time-step index. The $t\times t$ 
covariance matrix $\mathbf{C}$ is constructed as follows
\begin{equation}
\mathbf{C} = \frac{\mathbf{X}^T\mathbf{X}}{n-1}. 
\end{equation}
We can now diagonalize $\mathbf{C}$ to obtain 
\begin{equation}
...
containing the eigenvectors associated with the eigenvalues found in $\mathbf{L}$ (each column of $\mathbf{V}$ contains an eigenvector). The eigenvectors contained in 
$\mathbf{V}$ are also known as the prinicipal axes or directions of the data. The principal components are computed by projecting the data onto the principal axes. This is achieved 
via $\mathbf{Y} = \mathbf{X}\mathbf{V}$ (the dimensions of $\mathbf{Y}$ is the same as $\mathbf{X}$). The $j$th principal component is located in the $j$th column of $\mathbf{Y}$. To reduce the dimensionality of our 
data we simply need to remove some of the columns of $\mathbf{V}$. If we keep only the first two columns of $\mathbf{V}$ (denoted as $\mathbf{V}_2$), and we then project the data onto these two 
principal axes we obtain $\mathbf{Y}_2 = \mathbf{X}\mathbf{V}_2$ (i.e. the dimension of $\mathbf{Y}_2$ is $n\times 2$ and the dimension of $\mathbf{V}_2$ is $t\times 2$). 
...
\begin{equation}
H(P,Q) = 1 - \frac{|\mathbf{\Sigma}_1|^{\frac{1}{4}}|\mathbf{\Sigma}_2|^{\frac{1}{4}}}{|\frac{1}{2}(\mathbf{\Sigma}_1+\mathbf{\Sigma}_2)|^{\frac{1}{2}}}\exp \{-\frac{1}{8}(\mathbf{u}_1-\mathbf{u}_2)^T()^{-1}((\mathbf{u}_1-\mathbf{u}_2))\} 
\end{equation}
